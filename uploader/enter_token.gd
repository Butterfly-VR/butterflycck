...
func _on_token_entered() -> void:
	...
	var body: String = JSON.stringify(
		{ "email": email, "password_hash": password_hash as Array[int], "allow_renew": true }
	)
	api_handler \
...
func on_login_response(
	...
	var result: Array = api_handler.handle_response(
		code,
		body,
	...
	if result[0]:
		...
		account_handler.set_token(token, data["token_expires"], data["renewable"])
