...
const WARNING_LISTING: PackedScene = preload(
	"res://addons/butterflycck/uploader/inspector/warning.tscn"
)
...
func object_selected(original_root: BaseRoot, original_file_path: String) -> void:
	...
	for warning: BaseRoot.Warning in warnings:
		...
		listing.find_button.pressed.connect(
			func():
				...
				EditorInterface.edit_node(EditorInterface.get_edited_scene_root().get_node(path))
	...
	if root.attached_uuid:
		...
		if result[0]:
			...
			created_text.text = "Created at: " + Time.get_date_string_from_unix_time(
				values["created_at"] as int
			)
			modified_text.text = "Last modified: " + Time.get_date_string_from_unix_time(
				values["updated_at"] as int
			)
		else:
			name_text.text = (root.object_name if !root.object_name.is_empty() else root.name)
