...
const OBJECT_LISTING: PackedScene = preload(
	"res://addons/butterflycck/uploader/object list/object_listing.tscn"
)
...
func find_objects(type: GDScript) -> void:
	...
	for file: String in scene_files:
		...
		# fmt causes parse error here. can remove if it gets fixed
		# fmt: off
		load_thread.start(
			(func(file: String) -> PackedScene:
				return (load(file) as PackedScene)
				).bind(file) # unindenting this line causes the error
		)
		# fmt: on
...
func find_objects_in_scene(type: GDScript, scene_root: Node, origin_file_path: String) -> void:
	...
	EditorSceneTreeHelper.call_children_recursive(
		scene_root,
		check_node_is_object.bind(objects, type),
		true,
	)
	...
	for object: BaseRoot in objects:
		...
		listing.select_button.pressed.connect(inspector.object_selected.bind(
				object,
				origin_file_path,
			))
