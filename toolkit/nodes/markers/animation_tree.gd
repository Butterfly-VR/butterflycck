		...
		for i in range(0, animation.get_track_count()):
			if animation.track_get_type(i) == Animation.TrackType.TYPE_METHOD:
				warnings.append(BaseRoot.Warning.new(
						...
						"Animations are not allowed to call methods but %s has a method call track"
						% animation_name,
						tree.get_node(tree.anim_player),
						false,
					))
			if animation.track_get_type(i) == Animation.TrackType.TYPE_VALUE:
				warnings.append(BaseRoot.Warning.new(
						...
						"This may be supported in the future. offending animation: %s"
						% animation_name,
						tree.get_node(tree.anim_player),
						false,
					))
