...
func check_bone_config(target: Skeleton3D) -> bool:
	return (
		target.find_bone(head_bone) == -1 or target.find_bone(left_hand_bone) == -1
		or target.find_bone(right_hand_bone) == -1 or target.find_bone(left_foot_bone) == -1
		or target.find_bone(right_foot_bone) == -1 or target.find_bone(spine_bone) == -1
		or target.find_bone(hip_bone) == -1 or target.find_bone(chest_bone) == -1
	)


func check_bones_set() -> bool:
	return (
		head_bone == "" or left_hand_bone == "" or right_hand_bone == "" or left_foot_bone == ""
		or right_foot_bone == "" or spine_bone == "" or hip_bone == "" or chest_bone == ""
	)
